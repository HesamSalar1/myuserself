...
# تنظیمات اصلی بات 5
api_id = 25671829
api_hash = "YOUR_BOT5_API_HASH_HERE"
admin_id = 7346058093
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('bot5.log', encoding='utf-8'),
...
async def login_user():
    ...
    try:
        print(f"🔐 شروع فرآیند لاگین برای بات 5...")
        ...
        try:
            ...
            print(f"🆔 User ID: {me.id}")
            print(f"✅ Session ذخیره شد در: my_bot5.session")
...
def init_db():
    conn = sqlite3.connect('bot5_data.db')
    cursor = conn.cursor()
...
async def start_command(client, message: Message):
    await message.edit_text(f"🤖 **ربات 5 آماده است!**\n\n📋 برای لاگین: `/login`\n🆔 Admin: `{admin_id}`")
...
async def auto_reply_handler(client, message: Message):
    ...
    # بررسی دشمن بودن - بات 5 با تاخیر 0.004 ثانیه
    if user_id in enemy_cache and fosh_cache:
        selected = choice(fosh_cache)
        asyncio.create_task(send_delayed_reply(message, selected, 0.004))
        return

    # بررسی دوست بودن - بات 5 با تاخیر 0.004 ثانیه
...
async def bot_ready():
    ...
    logger.info("ربات 5 آماده شد و کش راه‌اندازی شد!")

@app.on_message(filters.command("help") & filters.user(admin_id))
async def help_command(client, message: Message):
    try:
        text = """🤖 **راهنمای ربات 5 - نسخه پیشرفته**
...
async def main():
    ...
    # بررسی وجود session
    if not os.path.exists("my_bot5.session"):
