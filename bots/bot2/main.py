...
async def auto_reply_handler(client, message: Message):
    try:
        logger.info(f"📨 BOT2 - پیام دریافت شد از {message.from_user.first_name if message.from_user else 'ناشناس'}")
        
        # بررسی فعال بودن پاسخگویی
        if not auto_reply_enabled:
            logger.info("⚠️ BOT2 - پاسخگویی خودکار غیرفعال است")
            return
            
        if not message.from_user:
            logger.info("⚠️ BOT2 - پیام بدون کاربر")
            return
            
        # بررسی نوع چت - فقط در گروه‌ها کار کن
        if message.chat.type == "private":
            logger.info(f"⚠️ BOT2 - پیام از چت خصوصی نادیده گرفته شد")
            return
            
        logger.info(f"📍 BOT2 - چت معتبر - نوع: {message.chat.type}")
            
        # نادیده گرفتن ادمین
        if message.from_user.id == admin_id:
            logger.info("⚠️ BOT2 - پیام از ادمین - نادیده گرفته شد")
            return
        ...
        logger.info(f"🔍 BOT2 - بررسی کاربر: {user_name} (ID: {user_id}) در گروه: {chat_title}")
        ...
        if user_id in enemy_ids:
            ...
            fosh_list = get_fosh_list()
            logger.info(f"🔥 BOT2 - تعداد فحش‌ها در دیتابیس: {len(fosh_list)}")
            
            if fosh_list:
                # انتخاب فحش تصادفی
                selected = choice(fosh_list)
                fosh_text, media_type, file_id = selected
                logger.info(f"🎲 BOT2 - فحش انتخاب شده: نوع={media_type or 'متن'}")

                try:
                    # ارسال فحش
                    if media_type and file_id:
                        logger.info(f"📤 BOT2 - در حال ارسال رسانه {media_type}")
                        if media_type == "photo":
                        ...
                        logger.info(f"📤 BOT2 - در حال ارسال متن: {fosh_text[:50]}")
                        await message.reply_text(fosh_text)
                    
                    logger.info(f"✅ BOT2 - فحش با موفقیت ارسال شد به {user_name}")
                    log_action("enemy_auto_reply", user_id, f"فحش به {user_name}")
                    
                except Exception as send_error:
                    logger.error(f"❌ BOT2 - خطا در ارسال فحش: {send_error}")
        ...
        if user_id in friend_ids:
            ...
            logger.info(f"💬 BOT2 - تعداد کلمات دوستانه: {len(friend_words)}")
            ...
            if friend_words:
                ...
                try:
                    # ارسال پیام دوستانه
                    if media_type and file_id:
                        logger.info(f"📤 BOT2 - در حال ارسال رسانه دوستانه {media_type}")
                        if media_type == "photo":
                            await message.reply_photo(file_id)
                        elif media_type == "video":
                            await message.reply_video(file_id)
                        ...
                        logger.info(f"📤 BOT2 - در حال ارسال متن دوستانه: {word_text[:50]}")
                        await message.reply_text(word_text)
                    
                    logger.info(f"✅ BOT2 - پیام دوستانه با موفقیت ارسال شد به {user_name}")
