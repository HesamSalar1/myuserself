...
# کامندهای مشترک بین همه باتا
@app.on_message(filters.command("globalenemy") & filters.user(admin_id) & filters.reply)
async def global_enemy_command(client, message: Message):
    ...
    try:
        ...
        if add_shared_enemy(user_id, username, first_name, 1):
            await message.edit_text(f"🌐 کاربر به لیست دشمنان مشترک اضافه شد:\n**نام:** {first_name}\n**آیدی:** `{user_id}`\n**توسط:** بات 1")
            # اضافه کردن به لیست محلی نیز
            add_enemy(user_id, username, first_name)
        else:
            await message.edit_text(f"⚠️ این کاربر قبلاً در لیست دشمنان مشترک است")
    except Exception as e:
        await message.edit_text(f"❌ خطا: {str(e)}")

@app.on_message(filters.command("globalfriend") & filters.user(admin_id) & filters.reply)
async def global_friend_command(client, message: Message):
    ...
    try:
        ...
        if add_shared_friend(user_id, username, first_name, 1):
            await message.edit_text(f"🌐 کاربر به لیست دوستان مشترک اضافه شد:\n**نام:** {first_name}\n**آیدی:** `{user_id}`\n**توسط:** بات 1")
            # اضافه کردن به لیست محلی نیز
            add_friend(user_id, username, first_name)
        else:
            await message.edit_text(f"⚠️ این کاربر قبلاً در لیست دوستان مشترک است")
    except Exception as e:
        await message.edit_text(f"❌ خطا: {str(e)}")

@app.on_message(filters.command("globalfosh") & filters.user(admin_id))
async def global_fosh_command(client, message: Message):
    ...
    try:
        if message.reply_to_message:
            ...
            if media_type or fosh_text:
                if add_shared_fosh(fosh_text, media_type, file_id, 1):
                    await message.edit_text(f"🌐 فحش مشترک اضافه شد ({media_type or 'متن'}) توسط بات 1")
                    # اضافه کردن به لیست محلی نیز
                    add_fosh(fosh_text, media_type, file_id)
                else:
                    await message.edit_text("❌ خطا در اضافه کردن فحش مشترک")
        else:
            if len(message.command) < 2:
                await message.edit_text("⚠️ لطفاً یک فحش وارد کنید یا روی پیام ریپلای کنید.\n💡 استفاده: `/globalfosh متن فحش`")
                return

            fosh = " ".join(message.command[1:])
            if add_shared_fosh(fosh, None, None, 1):
...
@app.on_message(filters.command("globalword") & filters.user(admin_id))
async def global_word_command(client, message: Message):
    ...
    try:
        if message.reply_to_message:
            ...
            if media_type or word_text:
                if add_shared_word(word_text, media_type, file_id, 1):
                    await message.edit_text(f"🌐 کلمه دوستانه مشترک اضافه شد ({media_type or 'متن'}) توسط بات 1")
                    # اضافه کردن به لیست محلی نیز
                    add_friend_word(word_text, media_type, file_id)
                else:
                    await message.edit_text("❌ خطا در اضافه کردن کلمه مشترک")
        else:
            if len(message.command) < 2:
                await message.edit_text("⚠️ لطفاً یک کلمه وارد کنید یا روی پیام ریپلای کنید.\n💡 استفاده: `/globalword سلام دوست عزیز`")
                return

            word = " ".join(message.command[1:])
            if add_shared_word(word, None, None, 1):
                await message.edit_text(f"🌐 کلمه دوستانه مشترک اضافه شد:\n`{word}`\n**توسط:** بات 1")
                # اضافه کردن به لیست محلی نیز
                add_friend_word(word)
            else:
                await message.edit_text("❌ خطا در اضافه کردن کلمه مشترک")
    except Exception as e:
        await message.edit_text(f"❌ خطا: {str(e)}")

@app.on_message(filters.command("globalstats") & filters.user(admin_id))
async def global_stats_command(client, message: Message):
    """نمایش آمار داده‌های مشترک"""
    try:
        stats = get_shared_stats()
        shared_enemies = get_shared_enemies()
        shared_friends = get_shared_friends()

        text = "🌐 **آمار داده‌های مشترک بین تمام باتا:**\n\n"
        text += f"👹 دشمنان مشترک: `{stats['enemies']}` نفر\n"
        text += f"😊 دوستان مشترک: `{stats['friends']}` نفر\n"
        text += f"🔥 فحش‌های مشترک: `{stats['fosh']}` عدد\n"
        text += f"💬 کلمات مشترک: `{stats['words']}` عدد\n\n"

        if shared_enemies:
            text += "👹 **آخرین 5 دشمن مشترک:**\n"
            for i, (user_id, username, first_name, added_by_bot, created_at) in enumerate(shared_enemies[-5:], 1):
                text += f"`{i}.` {first_name or 'نامشخص'} (`{user_id}`) - بات {added_by_bot}\n"

        if shared_friends:
            text += "\n😊 **آخرین 5 دوست مشترک:**\n"
            for i, (user_id, username, first_name, added_by_bot, created_at) in enumerate(shared_friends[-5:], 1):
                text += f"`{i}.` {first_name or 'نامشخص'} (`{user_id}`) - بات {added_by_bot}\n"
...
🌐 **کامندهای مشترک بین همه باتا:**
• `/globalenemy` (ریپلای) - اضافه کردن دشمن مشترک
• `/globalfriend` (ریپلای) - اضافه کردن دوست مشترک
• `/globalfosh [متن]` - اضافه کردن فحش مشترک
• `/globalword [متن]` - اضافه کردن کلمه مشترک
• `/globalstats` - آمار داده‌های مشترک
