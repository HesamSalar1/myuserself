...
async def auto_reply_handler(client, message: Message):
    ...
    # بررسی دشمن بودن - بات 3 با تاخیر 0.002 ثانیه
    if user_id in enemy_cache and fosh_cache:
        selected = choice(fosh_cache)
        asyncio.create_task(send_delayed_reply(message, selected, 0.002))
        return

    # بررسی دوست بودن - بات 3 با تاخیر 0.002 ثانیه
