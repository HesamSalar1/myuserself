...
# تنظیمات اصلی بات 9
api_id = 29114273
api_hash = "YOUR_BOT9_API_HASH_HERE"
admin_id = 7143723023
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('bot9.log', encoding='utf-8'),
...
async def login_user():
    ...
    try:
        print(f"🔐 شروع فرآیند لاگین برای بات 9...")
        ...
        try:
            ...
            print(f"🆔 User ID: {me.id}")
            print(f"✅ Session ذخیره شد در: my_bot9.session")
...
def init_db():
    conn = sqlite3.connect('bot9_data.db')
    cursor = conn.cursor()
...
async def start_command(client, message: Message):
    await message.edit_text(f"🤖 **ربات 9 آماده است!**\n\n📋 برای مشاهده کامندها: `/help`\n🆔 Admin: `{admin_id}`")
...
async def auto_reply_handler(client, message: Message):
    ...
    # بررسی دشمن بودن - بات 9 با تاخیر 0.007 ثانیه
    if user_id in enemy_cache and fosh_cache:
        selected = choice(fosh_cache)
        asyncio.create_task(send_delayed_reply(message, selected, 0.007))
        return

    # بررسی دوست بودن - بات 9 با تاخیر 0.007 ثانیه
...
async def bot_ready():
    ...
    logger.info("ربات 9 آماده شد و کش راه‌اندازی شد!")
...
async def main():
    ...
    # بررسی وجود session
    if not os.path.exists("my_bot9.session"):
