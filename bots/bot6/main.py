...
async def debug_system(client, message: Message):
    try:
        ...
        debug_info = f"""🔧 **دیباگ کامل سیستم BOT6:**
...
async def global_enemy_command(client, message: Message):
    ...
    try:
        ...
        if add_shared_enemy(user_id, username, first_name, 6):
...
async def global_friend_command(client, message: Message):
    ...
    try:
        ...
        if add_shared_friend(user_id, username, first_name, 6):
...
async def global_fosh_command(client, message: Message):
    ...
    try:
        if message.reply_to_message:
            ...
            if media_type or fosh_text:
                if add_shared_fosh(fosh_text, media_type, file_id, 6):
                    await message.edit_text(f"🌐 فحش مشترک اضافه شد ({media_type or 'متن'}) توسط بات 6")
                    # اضافه کردن به لیست محلی نیز
            ...
            fosh = " ".join(message.command[1:])
            if add_shared_fosh(fosh, None, None, 6):
...
async def global_word_command(client, message: Message):
    ...
    try:
        if message.reply_to_message:
            ...
            if media_type or word_text:
                if add_shared_word(word_text, media_type, file_id, 6):
                    await message.edit_text(f"🌐 کلمه دوستانه مشترک اضافه شد ({media_type or 'متن'}) توسط بات 6")
                    # اضافه کردن به لیست محلی نیز
            ...
            word = " ".join(message.command[1:])
            if add_shared_word(word, None, None, 6):
