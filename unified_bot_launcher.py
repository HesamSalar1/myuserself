...
class UnifiedBotLauncher:
    ...
    def add_forbidden_emoji_to_db(self, emoji, description="بدون توضیحات", category='custom', added_by_user_id=1842714289):
        """اضافه کردن ایموجی ممنوعه به دیتابیس - متد ساده"""
        try:
            import unicodedata
            ...
            # بررسی وجود قبلی
            cursor.execute("SELECT id FROM forbidden_emojis WHERE emoji = ?", (normalized_emoji,))
            existing = cursor.fetchone()
            
            if existing:
                print(f"⚠️ ایموجی '{emoji}' قبلاً در دیتابیس موجود است")
                conn.close()
                return False
            
            # اضافه کردن ایموجی جدید
            cursor.execute("""
                INSERT INTO forbidden_emojis (emoji, description, category, added_by_user_id, is_active)
                VALUES (?, ?, ?, ?, 1)
            """, (normalized_emoji, description, category, added_by_user_id))
            
            success = cursor.rowcount > 0
            
            if success:
                # به‌روزرسانی کش
                self.forbidden_emojis.add(normalized_emoji)
                print(f"✅ ایموجی '{emoji}' با موفقیت اضافه شد")
            ...
            print(f"❌ خطا در اضافه کردن ایموجی '{emoji}': {e}")
            return False
        ...
        try:
            ...
            async def add_emoji_command(client, message):
                try:
                    ...
                    if self.add_forbidden_emoji_to_db(emoji, description, 'custom', user_id):
                        await message.reply_text(f"✅ ایموجی ممنوعه اضافه شد: {emoji}\n📝 توضیحات: {description}")
                        self.log_action(bot_id, "add_emoji", user_id, f"ایموجی: {emoji}")
                    else:
                        await message.reply_text(f"❌ خطا در اضافه کردن ایموجی یا قبلاً موجود است: {emoji}")
            ...
            async def del_emoji_command(client, message):
                try:
                    ...
                    if success:
                        await message.reply_text(f"✅ ایموجی ممنوعه حذف شد: {emoji}")
                        self.log_action(bot_id, "del_emoji", user_id, f"ایموجی: {emoji}")
                    else:
                        await message.reply_text(f"❌ {msg}")
            ...
            async def list_emoji_command(client, message):
                try:
                    ...
                    if not emoji_list:
                        await message.reply_text("📝 لیست ایموجی‌های ممنوعه خالی است.\n💡 با `/addemoji` ایموجی اضافه کنید.")
                        return

                    text = "🚫 **لیست ایموجی‌های ممنوعه:**\n\n"
                    for i, emoji_data in enumerate(emoji_list[:20], 1):
                        emoji = emoji_data['emoji']
                        desc = emoji_data['description'] or 'بدون توضیحات'
                        text += f"`{i}.` {emoji} - {desc}\n"
            ...
            async def test_emoji_command(client, message):
                try:
                    ...
                    emoji = message.command[1]
                    test_text = f"تست: {emoji}"
                    
                    found_emojis = []
                    is_detected = self.contains_stop_emoji(test_text, found_emojis)
                    
                    if is_detected:
                        await message.reply_text(f"✅ ایموجی {emoji} تشخیص داده شد!\n🔍 یافت شده: {found_emojis[0] if found_emojis else 'نامشخص'}")
                    else:
                        await message.reply_text(f"❌ ایموجی {emoji} تشخیص داده نشد.\n💡 احتمالاً در لیست ممنوعه‌ها نیست یا مشکل در تشخیص وجود دارد.")
            ...
            async def debug_emoji_command(client, message):
                try:
                    if len(message.command) < 2:
                        await message.reply_text("⚠️ لطفاً متن تست را وارد کنید.\n💡 استفاده: `/debugemoji A CHARACTER HAS SPAWNED ⚡`")
                        return
                    ...
                    for char in test_text:
                        if ord(char) > 127:  # کاراکترهای غیر ASCII
                            unicode_info.append(f"'{char}' = U+{ord(char):04X}")
                    
                    result_text = f"🔍 **تست تشخیص ایموجی:**\n\n"
                    result_text += f"📝 **متن:** `{test_text}`\n"
                    result_text += f"🎯 **نتیجه:** {'✅ تشخیص داده شد' if is_detected else '❌ تشخیص داده نشد'}\n"
                    
                    if found_emojis:
                        result_text += f"🔍 **یافت شده:** {', '.join(found_emojis)}\n"
                    
                    if unicode_info:
                        result_text += f"\n🔤 **کاراکترهای Unicode:**\n"
                        for info in unicode_info[:10]:  # محدود به 10 کاراکتر
                            result_text += f"   {info}\n"
            ...
            async def sync_emojis_command(client, message):
                try:
                    ...
                    await message.reply_text(f"🔄 **همگام‌سازی ایموجی‌ها:**\n\n"
                                           f"📊 قبل: {old_count} ایموجی\n"
                                           f"📊 بعد: {new_count} ایموجی\n"
